...
export default function LaborerRegistrationForm({
  language = 'hi',
  registeredBy,
  submitLabel,
  onSuccess,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (registeredBy) {
        insertPayload.registered_by = registeredBy;
      }

      const { error: insertError } = await supabase.from('labourers').insert(insertPayload);
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-8">
      ...
      >
        {isSubmitting ? (
          ...
          <>
            <span>{submitLabel || t.submitProfile}</span>
